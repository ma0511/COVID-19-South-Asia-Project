...
###################################################
# Exploratory Analysis: Time Series of Daily Deaths
###################################################

df.time <- df %>%
  dplyr::select(deaths, date) %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(daily_deaths = sum(deaths, na.rm = TRUE))
...
df1 <- df %>%
  ...
  dplyr::summarise(deaths_dist = sum(deaths, na.rm = TRUE),
                   index = unique(index))
...
df$test <- ifelse(df$date >= "2021-02-01" & df$date <= "2021-03-31", 1, 0)
...
##... Model Performance : Using 2021 as the test
...
pROC::plot.roc(df1.test$death_cat, pred[,2], percent = FALSE, 
               main = "ROC curves for the CR-Index using Covid-19 deaths in Pakistan (Sindh)", 
