...
# df1$percentile_90 <- quantile(df1$index, probs = 0.90)
# 
# df1$case_cat <- with(df1, ave(cases_pct, c("district", "test"), FUN = function(x) ifelse(x >= quantile(x = cases_pct, probs = 0.75),
#                                                                                          "V.High", ifelse(x > quantile(x = cases_pct, probs = 0.25) & x < quantile(x = cases_pct, probs = 0.75), "Average","Low"))))
# df1$case_cat <- factor(df1$case_cat)
# df1$case_cat <- relevel(df1$case_cat, ref = "V.High")
