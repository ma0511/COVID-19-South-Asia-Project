...
##... Filter the data (select from March 2020)
...
df1 <- df2 %>%
  dplyr::select(ddeaths, district, month, year, index) %>%
  dplyr::group_by(district, month, year) %>%
  dplyr::summarise(deaths = sum(ddeaths, na.rm = TRUE),
                   index = unique(index))

df1 <- df1 %>% dplyr::group_by(month, year) %>% dplyr::mutate(total_deaths = sum(deaths))
df1 <- transform(df1, death_pct = (deaths/total_deaths) * 100)

df1$death_cat <- ifelse(df1$death_pct >= quantile(df1$death_pct, probs = 0.75), "High_Risk", "Low_Risk")
...
# 
# rf.cm.1 <- caret::confusionMatrix(rf.pred.1, df1.test$death_cat)
# rf.cm.1
# 
# df1.test <- cbind.data.frame(df1.test, as.data.frame(rf.pred.1))
# all(rf.pred.1 == df1.train[df1.train$month %in% df1.test$month, ]$death_cat)
# 
# 
# pred.1 <- predict(rf.cv.1 , df1.test, type = "prob")
# test.1 <- evalm(data.frame(pred.1, df1.test$death_cat))
# test.1

##... Model Performance: Model 2

rf.cv.2 <- train(death_cat ~ index,
                 data = df1.train,
                 method = 'rf',
                 trControl = control)
...
rf.cm.2 <- caret::confusionMatrix(rf.pred.2, df1.test$death_cat)
rf.cm.2

# df1.train$death_cat_dum <- ifelse(df1.train$death_cat == "High_Risk", 1, 0)
# table(df1.train$death_cat_dum)
# summary(lm(death_cat_dum ~ index, data = df1.train))
# summary(glm(death_cat_dum ~ index, data = df1.train, family = binomial(link = "probit")))


pred.2 <- predict(rf.cv.2 , df1.test, type = "prob")
test.2 <- evalm(data.frame(pred.2, df1.test$death_cat))
test.2

##... Model Performance: Model 3

# rf.cv.3 <- train(death_cat ~ month + index,
#                  data = df1.train,
#                  method = 'rf',
#                  trControl = control)
# 
...
# 
# rf.cm.3 <- caret::confusionMatrix(rf.pred.3, df1.test$death_cat)
# rf.cm.3
# 
# 
# pred.3 <- predict(rf.cv.3 , df1.test, type = "prob")
# test.3 <- evalm(data.frame(pred.3, df1.test$death_cat))
# test.3
...
df1$death_cat <- ifelse(df1$death_pct >= quantile(df1$death_pct, probs = 0.50), "High_Risk", "Low_Risk")
df1$death_cat <- factor(df1$death_cat)
...
df1 <- df1 %>% dplyr::group_by(month, year) %>% dplyr::mutate(total_deaths = sum(deaths))
df1 <- transform(df1, deaths_pct = (deaths/total_deaths) * 100)

df1$death_cat <- with(df1, ave(deaths_pct, FUN = function(x) ifelse(x >= quantile(x = deaths_pct, probs = 0.75), 
                                                                    "V.High", ifelse(x >= quantile(x = deaths_pct, probs = 0.50) & x < quantile(x = deaths_pct, probs = 0.75), "High",
                                                                                     ifelse(x >= quantile(x = deaths_pct, probs = 0.25) & x < quantile(x = deaths_pct, probs = 0.50), "Average",
                                                                                            "Low")))))

df1$death_cat <- factor(df1$death_cat)
df1$death_cat <- relevel(df1$death_cat, ref = "V.High")
...
pred.1 <- predict(rf.cv.1, df1.test, type = "prob")
pROC::multiclass.roc(df1.test$death_cat, pred.1)
