...
df1$death_cat <- with(df1, ave(deaths_pct, FUN = function(x) ifelse(x >= quantile(x = deaths_pct, probs = 0.90),
                                                                    "Red", ifelse(x >= quantile(x = deaths_pct, probs = 0.25) & x < quantile(x = deaths_pct, probs = 0.75), "Orange","Green"))))
