...
p1 <- ggplot(data = plotTable, mapping = aes(x = Reference, y = Prediction, fill = Accuracy, alpha = Freq)) +
  ...
  labs(title = "Confusion Matrix for the CR-Index using Covid-19 cases in India") + 
...
##... Test: Predictions using using cforest (Important). 
...
# cforest.cv <- train(case_cat ~ index,
#                     data = df1.train,
#                     method = "cforest",
#                     trControl = control)
# 
# cforest.pred <- predict(cforest.cv , df1.test)
# 
# cforest.cm <- caret::confusionMatrix(cforest.pred, df1.test$case_cat)
# cforest.cm
# 
# Matt_Coef(cforest.cm)

##... ROC - AUC

# pred <- predict(cforest.cv , df1.test, type = "prob")
# test <- evalm(data.frame(pred, df1.test$case_cat))
# test
# 
# df1.test <- cbind.data.frame(df1.test, as.data.frame(cforest.pred))
# View(cbind.data.frame(df1.test, pred = as.data.frame(pred)))
...
pred <- predict(xgb.cv, df1.test, type = "prob")
test <- evalm(data.frame(pred, df1.test$case_cat))
test
...
df1$case_cat <- with(df1, ave(cases_pct, c("district", "test"),
                              FUN = function(x) ifelse(x >= quantile(x = cases_pct, probs = 0.50), 
                                                       "High_Risk", "Low_Risk")))
df1$case_cat <- factor(df1$case_cat)
