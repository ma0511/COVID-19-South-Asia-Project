...
# df1$case_cat_dum <- ifelse(df1$case_cat == "High_Risk", 1, 0)
# table(df1$case_cat_dum)
# summary(lm(case_cat_dum ~ index, data = df1))
# summary(glm(case_cat_dum ~ index, data = df1, family = binomial(link = "probit")))
# 
# summary(lm(case_cat_dum ~ index + month, data = df1))
# summary(glm(case_cat_dum ~ index + month, data = df1, family = binomial(link = "probit")))
...
##... Test: Predictions using rpart (Important)
...
# rpart.cm
# 
# pred.2 <- predict(rpart.cv, df1.test, type = "prob")
# test.2 <- evalm(data.frame(pred.2, df1.test$case_cat))
# test.2
# 
# df1.test <- cbind.data.frame(df1.test, as.data.frame(rpart.pred))
# View(cbind.data.frame(df1.test, pred = as.data.frame(pred.2)))
