...
df.time <- df %>%
  dplyr::select(ddeaths, date) %>%
  dplyr::group_by(date) %>%
  dplyr::summarise(daily_deaths = sum(ddeaths, na.rm = TRUE))

df.time$month <- lubridate::month(df.time$date, label = TRUE, abbr = FALSE)
df.time$year <- lubridate::year(df.time$date)

ggplot(data = df.time, aes(x = as.Date(date), y = daily_deaths)) +
...
df <- dplyr::filter(df.ind, date >= "2020-03-01" & date <= "2021-01-31")
...
df1$death_cat <- with(df1, ave(deaths_pct, c("district", "test"),
                               FUN = function(x) ifelse(x >= quantile(x = deaths_pct, probs = 0.75), 
                                                        "High_Risk", "Low_Risk")))
...
test <- evalm(data.frame(pred, df1.test$death_cat))
test
...
df <- dplyr::filter(df.ind, date >= "2020-03-01" & date <= "2021-02-28")
...
df <- dplyr::filter(df.ind, date >= "2020-01-01" & date <= "2021-03-31")
...
df <- dplyr::filter(df.ind, date >= "2020-01-01" & date <= "2021-04-30")
...
df <- dplyr::filter(df.ind, date >= "2020-01-01" & date <= "2021-05-31")
...
df1$death_cat <- with(df1, ave(deaths_pct, test,
                               FUN = function(x) ifelse(x >= quantile(x = deaths_pct, probs = 0.75), 
                                                        "High_Risk", "Low_Risk")))
df1$death_cat <- factor(df1$death_cat)
...
df <- dplyr::filter(df.ind, date >= "2020-01-01" & date <= "2021-06-30")
...
df <- dplyr::filter(df.ind, date >= "2020-01-01" & date <= "2021-07-31")
...
df <- dplyr::filter(df.ind, date >= "2020-01-01" & date <= "2021-08-31")
...
df <- dplyr::filter(df.ind, date >= "2020-01-01" & date <= "2021-09-30")
...
df <- dplyr::filter(df.ind, date >= "2020-01-01" & date <= "2021-10-31")
...
pROC::plot.roc(df1.test$death_cat, pred[,2], percent = FALSE, 
               main = "ROC curves for the CR-Index using Covid-19 deaths in India", 
...
df1$death_cat <- with(df1, ave(deaths_pct, c("district", "test"),
                               FUN = function(x) ifelse(x >= quantile(x = deaths_pct, probs = 0.50), 
                                                        "High_Risk", "Low_Risk")))
df1$death_cat <- factor(df1$death_cat)
